...
class MinnesotaPrior(PriorDist):
    ...
    def calc_cov_param(self):
        ...
        self.phi_tilde = np.dot(self.Y.T, self.Y) + self.phi_bar + np.dot(
            np.dot(self.psi_bar.T, np.linalg.inv(self.omega_bar)), self.psi_bar) - np.dot(
            np.dot(self.psi_tilde.T, np.linalg.inv(self.omega_tilde)), self.psi_tilde)
