...
class VAR(ReducedModel):
    ...
    def irf(self, h: int) -> np.ndarray:
        self.irf_max_period = self._ReducedModel__get_irf(h=self.H, comp_mat=self.comp_mat, cov_mat=self.cov_mat)
        self.irf_point_estimate = self.irf_max_period[:, :h + 1]
        return self.irf_point_estimate

    def vd(self, h: int) -> np.ndarray:
        self.vd_point_estimate = self._ReducedModel__get_vd(self.irf_max_period[:, :h + 1])
    ...
    def bootstrap(self,
        ...
        for r in range(n_path):
            ...
            self.irf_mat[r, :, :] = irfr
            # TODO: next version, variance decomposition allows confidence interval
    ...
    def plot_irf(self,
        ...
        self._ReducedModel__make_irf_graph(h=h - 1, var_list=var_list, shock_list=shock_list,
